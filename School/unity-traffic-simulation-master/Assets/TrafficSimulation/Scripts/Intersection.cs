    ...
    {   
        ...
        void Start(){
            ...
            if(intersectionType == IntersectionType.TRAFFIC_LIGHT)
                // SwitchLights Method를 lightsDuration (8초) 후에 lightsDuration (8초)마다 실행 시켜라
                InvokeRepeating("SwitchLights", lightsDuration, lightsDuration);
        }


        void SwitchLights(){
            // currentRedLightsGroup을  1 → 2  OR 2 → 1로 변경
            ...
            // MoveVehiclesQueue()를 orangeLightDuration ( 2초 ) 후에 실행시켜라
